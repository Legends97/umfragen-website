            ...
            <li key={tally.script_id} className="flex items-start gap-3 rounded border p-4">
              ...
              <div className="flex-1 space-y-1">
                ...
                <p className="text-sm text-gray-600">
                  ...
                  {tally.not_needed_count} ({percent(tally.not_needed_count)}%)
                </p>
              </div>
            </li>
          );
            ...
            <tbody>
              {suggestions.map((suggestion) => (
                <tr key={suggestion.id} className="border-b">
                  ...
                  <td>
                    <a href={suggestion.link} target="_blank" className="text-blue-600 underline">
                      {suggestion.link}
                    </a>
                  </td>
                  <td>{suggestion.description ?? "—"}</td>
                  <td>{new Date(suggestion.created_at).toLocaleString("de-DE")}</td>
                </tr>
              ))}
            </tbody>
          </table>
